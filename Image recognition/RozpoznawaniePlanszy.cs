...
namespace Warcaby {
    public class RozpoznawaniePlanszy {
        private const string PLIK_KONFIGURACYJNY = @"C:\Users\Marcin\Desktop\Ustawienia.dat";
        private const int PLIK_DLUGOSC = 12;
        private const int ILOSC_KANALOW = 3;
        private const int ELEMENT_STRUKTURALNY1 = 6;
        private const int ELEMENT_STRUKTURALNY2 = 7;
        private const int ELEMENT_STRUKTURALNY3 = 6;
        ...
        private Capture Kamera = new Capture();
        ...
        public Point[] RozpoznajPionki() {
            if(Plansza == null) return null;

            Mat obr = WczytajObraz();
            VectorOfVectorOfPoint kontury = new VectorOfVectorOfPoint();
            Mat wyciety = new Mat();
            AnalizujPlansze(obr, pionki_min, pionki_max, ref kontury, ref wyciety);

            Point[][] punkty = kontury.ToArrayOfArray();
            Pole pole;
            Point srodek;
            Pole pp;
            List<Point> pionki = new List<Point>();
            bool znaleziono;
        ...
        public void Kalibruj(TypObiektu element) {
            ...
            if(element == TypObiektu.Pionki) {
                okno = new wndKalibracja(pionki_min.H, pionki_min.S, pionki_min.V, pionki_max.H, pionki_max.S, pionki_max.V, "Ustaw zakres tak, aby na obrazie widoczne były tylko pionki.");
            } else {
                okno = new wndKalibracja(pola_min.H, pola_min.S, pola_min.V, pola_max.H, pola_max.S, pola_max.V, "Ustaw zakres tak, aby na obrazie widoczne były tylko białe pola.");
            }
            ...
            if(okno.DialogResult == DialogResult.OK) {
                if(element == TypObiektu.Pionki) {
                    pionki_min = new KolorHSV(okno.Hmin, okno.Smin, okno.Vmin);
                    pionki_max = new KolorHSV(okno.Hmax, okno.Smax, okno.Vmax);
                } else {
                    pola_min = new KolorHSV(okno.Hmin, okno.Smin, okno.Vmin);
                    pola_max = new KolorHSV(okno.Hmax, okno.Smax, okno.Vmax);
                }
                ZapiszUstawienia();
            }
        }

        public enum TypObiektu { Pionki, Pola };
        ...
        public void PokazObraz() {
            ...
            NamedWindow(tytul);
            MCvScalar kolor_pionki = new MCvScalar(255, 0, 0);
            MCvScalar kolor_pola = new MCvScalar(0, 255, 0);
            MCvScalar kolor_granice = new MCvScalar(0, 0, 255);

            while(PokazywanieObrazu) {
                VectorOfVectorOfPoint kontury_pionki = new VectorOfVectorOfPoint();
                VectorOfVectorOfPoint kontury_pola = new VectorOfVectorOfPoint();
                Mat wyciety1 = new Mat();
                Mat wyciety2 = new Mat();
                Mat obr = WczytajObraz();
                Mat obr2 = obr.Clone();
                CvtColor(obr2, obr2, ColorConversion.Hsv2Bgr);

                AnalizujPlansze(obr, pionki_min, pionki_max, ref kontury_pionki, ref wyciety1);
                AnalizujPlansze(obr, pola_min, pola_max, ref kontury_pola, ref wyciety2);

                for(int i = 0; i < kontury_pionki.Size; i++) {
                    DrawContours(obr2, kontury_pionki, i, kolor_pionki, 5);
                }

                for(int i = 0; i < kontury_pola.Size; i++) {
                    DrawContours(obr2, kontury_pola, i, kolor_pola, 5);
                }
